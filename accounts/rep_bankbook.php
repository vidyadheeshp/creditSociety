...
$PanelHeading = "Bank Book From ".$FromDate. " To ". $ToDate;
...
if(isset($_POST['fromdate'])){
	...
	if($Destination=='Display'){
		...
		$Header .= "Bank Book : <strong>".$FMName. " AccID:".$FMID." </strong>From ".$RepFromDate. " To ".$RepToDate."<br>";
		$Html  = "";
		...
		foreach($OuterResultSet as $OuterRow){
			...
			foreach($Inner1ResultSet as $Inner1Row){
				...
				$SubAccName = "";
				$Particulars= "";
				$Particulars= $Inner1Row['Particulars'];
				...
				$Html .= "<td>".date("d-m-Y",strtotime($ProcDate))."</td>";
				$Html .= "<td>".$Inner1Row['TrnCode']."</td>";
				$Html .= "<td>".$Inner1Row['FMID']."</td>";
				$Html .= "<td>".$FtFMName." ".$SubAccID." ".$SubAccName." ".$Particulars."</td>";
				$Html .= "<td align='right'>".number_format($DayReceipt,2, '.', '')."</td>";
				$Html .= "<td align='right'>".number_format($DayPayment,2, '.', '')."</td>";
				$Html .= "</tr>";
			}
			...
			foreach($Inner2ResultSet as $Inner2Row){
				...
				$Html .="<tr  style='background-color:lightblue;'>";
				$Html .= "<td>".date("d-m-Y",strtotime($ProcDate))."</td>";
				$Html .= "<td>".$Inner2Row['TrnCode']."</td>";
				$Html .= "<td>".$Inner2Row['FMID']."</td>";
				$Html .= "<td>".$FtFMName." ".$SubAccID." ".$SubAccName."</td>";
				$Html .= "<td align='right'>".number_format($DayReceipt,2, '.', '')."</td>";
				$Html .= "<td align='right'>".number_format($DayPayment,2, '.', '')."</td>";
				$Html .= "</tr>";
			}
			// print daily total and balance
				...
				$Html .= "<td align='right'>".number_format($DayTotReceipt,2, '.', '')."</td>";
				$Html .= "<td align='right'>".number_format($DayTotPayment,2, '.', '')."</td>";
				$Html .= "</tr>";
			// print daily balance
				$Html .="<tr  style='background-color:pink;'>";
				$Html .= "<td></td>";
		...
		foreach($OuterResultSet as $OuterRow){
			...
			foreach($Inner1ResultSet as $Inner1Row){
				...
				$Html .= date("d-m-Y",strtotime($ProcDate))."\t";
				$Html .= $Inner1Row['TrnCode']."\t";
				$Html .= $Inner1Row['FMID']."\t";
				$Html .= $FtFMName." ".$SubAccID." ".$SubAccName."\t";
				$Html .= number_format($DayReceipt,2, '.', '')."\t";
				$Html .= number_format($DayPayment,2, '.', '')."\t";
				$Html .= "\n";
			}
			...
			// print daily total and balance
				$Html .= "\t\t\t";
				$Html .= "Day Total\t";
				$Html .= number_format($DayTotReceipt,2, '.', '')."\t";
				$Html .= number_format($DayTotPayment,2, '.', '')."\t";
				$Html .= "\n";
			// print daily balance
				$Html .= "\t\t\t";
				$Html .= "Day Balance\t";
		...
		$Report = $Header . $Html;
		CreateLog("Excel Bank Book Created");
			...
			<div class='panel panel-primary'>
				...
				<div class='panel-body'>
					<div class='row form-inline hidden-print'>
						<form id='form1' name='form1' action='rep_bankbook.php' method='post'>
						<?php echo $FMList; ?>
