...
$FromDate     = date("Y-m-d");
$ToDate       = date("Y-m-d");
$PanelHeading = "Trial Balance As On ".$ToDate;
if(isset($_POST['asondate'])){
	...
	$Sql = "Select A.PlBsCode,A.FMID,A.Name,A.OpenBal,
			(Select SUM(Debit)  from ft Where A.FMID=ft.FMID    AND ft.TrnDate<='$AsOnDate') as Debit1,
			(Select SUM(Credit) from ft Where A.FMID=ft.FMID    AND ft.TrnDate<='$AsOnDate') as Credit1,
			(Select SUM(Debit)  from ft Where A.FMID=ft.FixFMID AND ft.TrnDate<='$AsOnDate') as Credit2,
			(Select SUM(Credit) from ft Where A.FMID=ft.FixFMID AND ft.TrnDate<='$AsOnDate') as Debit2
		From fm A 
	...
	if($Destination=='Display'){
		...
		$Html .= "S.No\tName of Account\tDebit\tCredit\t\n";
		$ResultRow = getResultSet($db,$Sql);
		$SerialNo 	= 1;
		$TotDebit 	= 0;
		$TotCredit 	= 0;
    	...
    	<div class="col-md-6 col-md-offset-3">
			...
			<div class='panel panel-primary'>
				...
				<div class='panel-body'>
					<form id='form1' name='form1' action='rep_tb1.php' method='post'>
					<div class='row form-inline'>
						<label>Enter As On Date</label>
						<input type='date' id='asondate' name='asondate' value='<?php echo $ToDate;?>' />
