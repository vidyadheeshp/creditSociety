<?php
	...
	CreateLog("inside divdcalc.php  FinYear $FinYear Div $DivPer $DateStr1 $DateStr2");
	...
	$TotDeb = 0;
	$TotCre = 0;
	$ShareFMID = getSingleField($db,"Select FMID from fm Where AcType='SC'");
	...
	foreach($Rs as $MemberRow){
		...
		//$Balance = getOpenBalShares($db,$ShareFMID,$MemberID,$FromDate);

		//

		$Str = "Select SUM(Credit-Debit) From ft Where FMID='$ShareFMID' and MemberID='$MemberID' 
		...
		// for saving data
		$Array = array("FinYear"=>$FinYear,"MemberID"=>$MemberID,"FromDate"=>$FromDate,"ToDate"=>$ToDate,"DivPer"=>$DivPer,"DivAmt"=>$Dividend,"OpenBal"=>$OpenBal,"Debits"=>$IndDeb,"Credits"=>$IndCre,"ClosBal"=>$Balance,"DeptID"=>$DeptID);
					...
					<div class='row'>
						<table id='table0'>
							<tr style='background-color:brown;color:white'>
								...
								<td><label><?php echo $DivPer;?></label></td>
							</tr>
						</table>						
					</div>
...
<script>
	var divper  = <?php echo $DivPer;?>;
	var finyear = <?php echo $FinYear;?>;
	function js_excel1() {
		window.location.href='ex_dividend1.php?FinYear='+finyear+'&DivPer='+divper;
	}
	function js_excel2() {
		window.location.href='ex_dividend2.php?FinYear='+finyear+'&DivPer='+divper;
	}
	function js_excel3() {
		window.location.href='ex_dividend3.php?FinYear='+finyear+'&DivPer='+divper;
	}
</script>
</html>