...
//	Trial Balance ShareHolders Accounts
...
$ToDate       = date("Y-m-d");
$AsOnDate     = date("Y-m-d");
$PanelHeading = "ShareHolders Trial Balance As On ".$AsOnDate;
...
if(isset($_POST['asondate'])){
	...
	$Sql = "Select A.DeptID,A.MemberID,A.Name,A.OpenBal,
			(Select SUM(Debit)  from ft Where A.MemberID=ft.MemberID    AND (ft.TrnDate <='$AsOnDate')) as Debit1,
			(Select SUM(Credit) from ft Where A.MemberID = ft.MemberID    AND (ft.TrnDate <= '$AsOnDate')) as Credit1
		From shareholders A 
	...
	if($Destination=='Display'){
		...
		$Header .= "ShareHolders Trial Balance As On : <strong>".$RepAsOnDate." </strong>";
		$Html  = "";
		...
		$Html .= "ShareHolders Trial Balance As On : ".$RepAsOnDate."\n";
		$Html .= "S.No\t
				Dept\t
				MemberID\t
				Name of Account\t
	    ...
	    header("Content-Disposition: attachment; filename=shares_tb.xls");
    	...
    	<div class="col-md-10 col-md-offset-1">
			...
			<div class='panel panel-primary'>
				...
				<div class='panel-body'>
					<form id='form1' name='form1' action='rep_tb3.php' method='post'>
